...
c 1002 format
c     +  (/,'At iterate',i5,4x,'f= ',1p,d16.10,4x,'|proj g|= ',1p,d12.5)
1002  format
     +  (' At iterate',i5,4x,'Uel= ',3p,d16.10,4x,'|Proj g|= ',
     +   1p,d12.5,/)
...
c                   this variable is always fixed
  ...
  20  continue
      ...
      if (iprint .ge. 0) then
         !if (prjctd) write (6,*)
     +   !'The initial X is infeasible.   projection.'
         !if (.not. cnstnd)
 ...
 !1001 format (/,'At initial X',i9,'variables are exactly at the bounds') 
 1001 format(/,'At initial X',i9,' variables are exactly at the bounds')
      return
...
c     ************
 ...
 3003 format (/,
     ...
     + 'Tnf   = total number of Uel (function f) evaluations',/,
     ...
     + 'Uel   = final Uel (function f) value',/,/,
     + '           * * *')
 3004 format (/,3x,'N',4x,'Tit',5x,'Tnf',2x,'Tnint',2x,
     +       'Skip',2x,'Nact',5x,'Projg',8x,'Uel')
